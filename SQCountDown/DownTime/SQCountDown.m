...
@interface SQCountDown ()
@property(nonatomic,retain) dispatch_source_t timer;
@property(nonatomic,retain) NSDateFormatter *dateFormatter;

@end

@implementation SQCountDown
- (instancetype)init{
    self = [super init];
    if (self) {
...
-(void)countDownWithPER_MSECBlock:(void (^)())PER_MSECBlock{
    if (_timer==nil) {
        dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        _timer = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0,queue);
...
- (void)countDownStartateStr:(NSString *)startTime expirationDays:(NSInteger)days completeBlock:(void (^)(NSInteger day,NSInteger hour,NSInteger minute,NSInteger second))completeBlock{
    //结束时间
    NSDate *endDate = [self getGoodsExpireDate1:days startTime:startTime];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    dateFormatter.dateFormat = @"YYYY-MM-dd HH:mm:ss";
    //开始时间
    NSDate *startDate = [dateFormatter dateFromString:startTime];
//    NSInteger startTimeSp = [[NSNumber numberWithDouble:[startDate timeIntervalSince1970]] integerValue]*1000;
    [self countDownWithStratDate:startDate finishDate:endDate completeBlock:^(NSInteger day, NSInteger hour, NSInteger minute, NSInteger second) {
